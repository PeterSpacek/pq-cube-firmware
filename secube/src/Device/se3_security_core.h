...
typedef struct se3_key_pair_ {
	se3_flash_key pk;
	se3_flash_key sk;
} se3_key_pair;


/** \brief crypto_kem_keypair function type */
typedef uint16_t(*se3_crypto_kem_keypair_handler)(
	uint8_t* pk, uint8_t* sk);

/** \brief crypto_kem_enc function type */
typedef uint16_t(*se3_crypto_kem_enc_handler)(
		uint8_t *ct, uint8_t *ss, const uint8_t *pk);

/** \brief crypto_kem_enc function type */
typedef uint16_t(*se3_crypto_kem_dec_handler)(
		uint8_t *ss, const uint8_t *ct, const uint8_t *sk);

/** \brief algorithm descriptor type */
typedef struct se3_kem_descriptor_ {
	se3_crypto_kem_keypair_handler keypair;  ///< crypto_kem_keypair function
	se3_crypto_kem_enc_handler enc;  ///< crypto_kem_enc function
	se3_crypto_kem_dec_handler dec;  ///< crypto_kem_dec function
	char display_name[16];  ///< name for the algorithm list API
	uint16_t display_type;  ///< type for the algorithm list API
	uint16_t display_size;  ///< block size for the algorithm list API
	uint16_t display_ciphertext_size;  ///< block size for the algorithm list API
	uint16_t display_public_key_size;  ///< key size for the algorithm list API
	uint16_t display_secret_key_size;  ///< key size for the algorithm list API
} se3_kem_descriptor;
