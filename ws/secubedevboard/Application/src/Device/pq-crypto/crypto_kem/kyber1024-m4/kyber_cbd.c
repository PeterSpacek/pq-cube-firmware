...
**************************************************/
static uint32_t kyber_load32_littleendian(const unsigned char *x) {
    uint32_t r;
    r  = (uint32_t)x[0];
    r |= (uint32_t)x[1] << 8;
    r |= (uint32_t)x[2] << 16;
...
**************************************************/
void kyber_cbd(poly *r, const unsigned char *buf, int add) {
    uint32_t d, t;
    int16_t a, b;
    int i, j;

    for (i = 0; i < KYBER_N / 8; i++) {
        t = kyber_load32_littleendian(buf + 4 * i);
        d  = t & 0x55555555;
        d += (t >> 1) & 0x55555555;
