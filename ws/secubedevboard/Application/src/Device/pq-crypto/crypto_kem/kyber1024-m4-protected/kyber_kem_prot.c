...
int KYBER1024_M4_crypto_kem_keypair_prot(unsigned char *pk, unsigned char *sk) {
    size_t i;
    indcpa_keypair_prot(pk, sk);
    for (i = 0; i < KYBER_INDCPA_PUBLICKEYBYTES; i++) {
        sk[i + KYBER_INDCPA_SECRETKEYBYTES] = pk[i];
    }
    hash_h_prot(sk + KYBER_SECRETKEYBYTES - 2 * KYBER_SYMBYTES, pk, KYBER_PUBLICKEYBYTES);
...
int KYBER1024_M4_crypto_kem_enc_prot(unsigned char *ct, unsigned char *ss, const unsigned char *pk) {
    ...
    hash_h_prot(buf, buf, KYBER_SYMBYTES);                                        /* Don't release system RNG output */

    hash_h_prot(buf + KYBER_SYMBYTES, pk, KYBER_PUBLICKEYBYTES);                  /* Multitarget countermeasure for coins + contributory KEM */
    hash_g_prot(kr, buf, 2 * KYBER_SYMBYTES);

    indcpa_enc_prot(ct, buf, pk, kr + KYBER_SYMBYTES);                            /* coins are in kr+KYBER_SYMBYTES */

    hash_h_prot(kr + KYBER_SYMBYTES, ct, KYBER_CIPHERTEXTBYTES);                  /* overwrite coins in kr with H(c) */
    kdf_prot(ss, kr, 2 * KYBER_SYMBYTES);                                         /* hash concatenation of pre-k and H(c) to k */
    return 0;
}
...
int KYBER1024_M4_crypto_kem_dec_prot(unsigned char *ss, const unsigned char *ct, const unsigned char *sk) {
    ...
    fail = indcpa_enc_cmp_prot(ct, buf, pk, kr + KYBER_SYMBYTES);                          /* coins are in kr+KYBER_SYMBYTES */

    hash_h_prot(kr + KYBER_SYMBYTES, ct, KYBER_CIPHERTEXTBYTES);                           /* overwrite coins in kr with H(c)  */

    kyber_cmov_prot(kr, sk + KYBER_SECRETKEYBYTES - KYBER_SYMBYTES, KYBER_SYMBYTES, fail);       /* Overwrite pre-k with z on re-encryption failure */
