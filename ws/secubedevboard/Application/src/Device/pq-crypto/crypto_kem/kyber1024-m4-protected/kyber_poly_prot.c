...
extern void doublebasemul_asm_prot(int16_t *r, const int16_t *a, const int16_t *b, int16_t zeta);
/*************************************************
...
void poly_frombytes_mul_prot(poly *r, const unsigned char *a) {
    ...
    for (i = 0; i < KYBER_N / 4; i++) {
        ...
        doublebasemul_asm(&r->coeffs[4*i], &r->coeffs[4*i], tmp, zetas_poly[i]);
    }
}
...
void poly_noise_prot(poly *r, const unsigned char *seed, unsigned char nonce, int add)
{
    ...
    prf_prot(buf, KYBER_ETA * KYBER_N / 4, seed, nonce);
    kyber_cbd_prot(r, buf, add);
}
...
extern unsigned long long randomize_ntt_total_time, t0_total_ntt, t1_total_ntt;
extern unsigned long long randomize_intt_total_time, t0_total_intt, t1_total_intt;

void poly_ntt_prot(poly *r, int protection)
{
  if(protection == 1)
  {
    #if (CALC_TOTAL_NTT_TIME == 1)
      t0_total_ntt = hal_get_time();
    #endif

    ntt_prot(r->coeffs);
    #if (CALC_TOTAL_NTT_TIME == 1)
      t1_total_ntt = hal_get_time();
      randomize_ntt_total_time = t1_total_ntt - t0_total_ntt;
    #endif
  }
  else
  {
...
void poly_invntt_prot(poly *r, int protection)
{
  ...
  {
    #if (CALC_TOTAL_NTT_TIME == 1)
      t0_total_intt = hal_get_time();
    #endif

    invntt_prot(r->coeffs);

    #if (CALC_TOTAL_NTT_TIME == 1)
      t1_total_intt = hal_get_time();
      randomize_intt_total_time = t1_total_intt - t0_total_intt;
    #endif
  }
  else
  {
...
void poly_basemul_prot(poly *r, const poly *a, const poly *b) {
    basemul_asm(r->coeffs, a->coeffs, b->coeffs, zetas_poly);
}
...
void poly_basemul_acc_prot(poly *r, const poly *a, const poly *b) {
    basemul_asm_acc(r->coeffs, a->coeffs, b->coeffs, zetas_poly);
}
