...
**************************************************/
void polyvec_compress_prot(unsigned char *r, polyvec *a)
{
  int i,j,k;
...
**************************************************/
void polyvec_decompress_prot(polyvec *r, const unsigned char *a)
{
  int i,j;
#if (KYBER_POLYVECCOMPRESSEDBYTES == (KYBER_K * 352))
  for(i=0;i<KYBER_K;i++)
...
*              - const polyvec *a: pointer to input vector of polynomials
**************************************************/
void polyvec_tobytes_prot(unsigned char *r, polyvec *a)
{
  int i;
  for(i=0;i<KYBER_K;i++)
    poly_tobytes_prot(r+i*KYBER_POLYBYTES, &a->vec[i]);
}
...
**************************************************/
void polyvec_frombytes_prot(polyvec *r, const unsigned char *a)
{
  int i;
  for(i=0;i<KYBER_K;i++)
    poly_frombytes_prot(&r->vec[i], a+i*KYBER_POLYBYTES);
}
...
**************************************************/
void polyvec_ntt_prot(polyvec *r, int protection)
{
  int i;
  for(i=0;i<KYBER_K;i++)
    poly_ntt_prot(&r->vec[i], protection);
}
...
* Arguments:   - polyvec *r: pointer to in/output vector of polynomials
**************************************************/
void polyvec_invntt_prot(polyvec *r, int protection)
{
  int i;
  for(i=0;i<KYBER_K;i++)
    poly_invntt_prot(&r->vec[i], protection);
}
...
**************************************************/
void polyvec_reduce_prot(polyvec *r)
{
  int i;
  for(i=0;i<KYBER_K;i++)
    poly_reduce_prot(&r->vec[i]);
}
...
**************************************************/
void polyvec_add_prot(polyvec *r, const polyvec *a, const polyvec *b)
{
  int i;
  for(i=0;i<KYBER_K;i++)
    poly_add_prot(&r->vec[i], &a->vec[i], &b->vec[i]);
}
