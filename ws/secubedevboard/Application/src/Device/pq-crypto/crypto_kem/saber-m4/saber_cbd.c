...
#if Saber_type == 1
static uint64_t saber_load_littleendian(const unsigned char *x, int bytes)
{
  int i;
  uint64_t r = x[0];
  for(i=1;i<bytes;i++)
...
static uint32_t saber_load_littleendian(const unsigned char *x, int bytes)
{
  int i;
  uint32_t r = x[0];
  for(i=1;i<bytes;i++)
...
void saber_cbd(uint16_t *r, const unsigned char *buf)
{
  ...
  {
    t = saber_load_littleendian(buf+3*i,3);
    d = 0;
    for(j=0;j<3;j++)
      d += (t >> j) & 0x249249;
